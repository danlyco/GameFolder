...
class DisplayManager {
   ...
   */
  constructor() {
    this.state = 'intro'; // eslint-disable-next-line no-restricted-syntax
  ...
  intro() {
    ...
    $('<button></button>').text('Nouvelle partie').addClass('intro--newgame').appendTo($intro);
    $('<img>').addClass('intro--leaves').attr('src', './assets/img/intro-leaves.png').appendTo($intro);
    $('<img>').addClass('intro--title').attr('src', './assets/img/intro-title.png').appendTo($intro);
  }
  ...
  characterSelection(mode) {
    ...
    [1, 2].forEach(elem => {
      ...
      for (const char of possibleChar) {
        $('<div></div>').addClass('intro--characterSelection__char').css('background-image', `url('./assets/chars/${char.toLowerCase()}.png')`).appendTo($charSelChars);
      }
  ...
  init() {
    ...
    const $gameBoard = $('tbody'); // Calculating number of rows and line
    ...
    for (let y = 0; y < numberRowCol; ++y) {
      ...
      for (let x = 0; x < numberRowCol; ++x) {
        ...
        if (Game.board.tiles[tileNumber].isEmpty()) {
          ...
          } else {// $block.find('.item').css('opacity', '0');
          }
        }
    ...
    $(`.tile-${Game.players[0].char.getTile()}`).find('div:eq(0)').removeAttr('class').addClass('block--player').addClass(`block--player__${Game.players[0].char.name.toLowerCase()}`);
    $(`.tile-${Game.players[1].char.getTile()}`).find('div:eq(0)').removeAttr('class').addClass('block--player').addClass(`block--player__${Game.players[1].char.name.toLowerCase()}`);
    Display.interface();
  }

  static showDamages() {} // let $;
