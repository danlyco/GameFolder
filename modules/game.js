...
class Observable {
  ...
  initProperties() {
    ...
    for (const key of Object.keys(this)) {
      const value = this[key]; // eslint-disable-next-line no-continue
...
class Board {
  ...
  isPlayerNear(playerTile) {
    ...
    adjacentTiles.push(this.getTile(tile._x, tile._y - 1)); // eslint-disable-next-line no-restricted-syntax
...
class Survivor extends Character {
  constructor(x, y) {
    super(x, y);
    this.model = ['Adam']; // eslint-disable-next-line prefer-destructuring
...
class Chaser extends Character {
  constructor(x, y) {
    super(x, y);
    this.model = ['Billy', 'Myers', 'Clown']; // eslint-disable-next-line prefer-destructuring
...
class Player {
  ...
  constructor(number, desiredChar, desiredModel) {
    ...
    if (desiredModel instanceof String) this.char.name = desiredModel;else this.char.name = this.char.model[desiredModel];
  }
...
class GameManager extends Observable {
  ...
  constructor() {
    ...
    this.playersAction = []; // Define active player based on character selection
    // Survivor is always starting first

    this.players[0].char instanceof Survivor ? this.activePlayer = 0 : this.activePlayer = 1;
    this.totalTurn = 0; // Define turns required before survivor escape
  ...
  playerSpawn() {
    ...
    for (const [index, elem] of this.players.entries()) {
      ...
      let randomTile = Math.floor(Math.random() * this.board.tiles.length); // Prevent player from spawning on a player or an obstacle

      while (this.board.tiles[randomTile] instanceof Obstacle || this.board.tiles[randomTile].hasPlayer === 1) {
  ...
  nextTurn(action) {
    ...
    if (this.players[0].char.onFight === 0) {
      this.players[this.activePlayer].char.movementPoint = this.players[this.activePlayer].char.defaultMovePoints;
    } else {
      const playerAction = {
    ...
    this.activePlayer === 0 ? this.activePlayer = 1 : this.activePlayer = 0; // Increase total Game turn each time both players turn ended
  ...
  nextGameTurn() {
    this.totalTurn = this.turn / 2; // Respawn item pool each 3 turns
  ...
  getPossibleMoves() {
    this.removePossibleMoves(); // ANCHOR

    this.possibleBottomMovesTiles = [];
    this.possibleTopMovesTiles = [];
    this.possibleLeftMovesTiles = [];
    this.possibleRightMovesTiles = [];
    this.possibleMovesIterator = [this.possibleBottomMovesTiles, this.possibleRightMovesTiles, this.possibleLeftMovesTiles, this.possibleTopMovesTiles]; // eslint-disable-next-line no-restricted-syntax
      ...
      for (let i = 1; i <= this.players[this.activePlayer].char.movementPoint; i++) {
        ...
        switch (elem) {
          case this.possibleBottomMovesTiles:
            elem[i] = this.board.getTile(this.players[this.activePlayer].char._x, this.players[this.activePlayer].char._y + i);
            break;
          ...
          case this.possibleLeftMovesTiles:
            elem[i] = this.board.getTile(this.players[this.activePlayer].char._x - i, this.players[this.activePlayer].char._y);
            break;
...
class VersusGameManager extends GameManager {
  ...
  constructor() {
    super();
    this.players = [new Player(1, 2, 0), new Player(2, 2, 2)]; // eslint-disable-next-line no-restricted-syntax
      ...
      if (this.players[this.playersAction[0].player].char._health > 0) {
        if (this.playersAction[0].action === 'attack') {
          if (this.playersAction[1].action === 'defend') {
            this.players[this.playersAction[1].player].char._health -= this.players[this.playersAction[0].player].char._equipedItem.amount / 2;
          } else {
          ...
          this.players[this.playersAction[0].player].char._health -= this.players[this.playersAction[1].player].char._equipedItem.amount / 2;
        }
      }

      this.players[this.playersAction[0].player].char._health = Math.floor(this.players[this.playersAction[0].player].char._health);
      this.players[this.playersAction[1].player].char._health = Math.floor(this.players[this.playersAction[1].player].char._health);
